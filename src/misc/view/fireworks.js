...
var fireworksSHOWER = {
  ...
  // random colors
  _color: config.options.fireworks.color,
  // gravity factor
  _gravity: config.options.fireworks.gravity,
  // air resistance factor
  _resistance: config.options.fireworks.resistance,
  // zIndex of particles
  _zIndex: 20000,
  // maximal age of particles (in msec)
  _maxAge: config.options.fireworks.maxAge,
  // interval of appearing explosions (in msec)
  _interval: config.options.fireworks.interval,
  // amount of particles per explosion
  _particlesPerExplosion: config.options.fireworks.particlesPerExplosion,
  // maximal speed of particle at moment of explosion
  _speed: config.options.fireworks.speed,
  // minimal/maximal size of particle
  _minSize: config.options.fireworks.minSize,
  _maxSize: config.options.fireworks.maxSize,
  ...
  init: function () {
    this._addEventListener(window, "resize", function () {
      return fireworksSHOWER.resize.apply(fireworksSHOWER);
    });
    this._addEventListener(window, "load", function () {
      return fireworksSHOWER.start.apply(fireworksSHOWER);
    });
  },
  ...
  _addExplosion: function () {
    ...
    window.setTimeout(function () {
      return fireworksSHOWER._addExplosion.apply(fireworksSHOWER);
  ...
  _move: function () {
    ...
    for (var i = 0, l = p.length; i < l; i++) {
      ...
      el.style.top = d.y + "px";
      el.style.color =
        Math.random() * 0.5 + d.age >= 1 ? "transparent" : d.color;
      el.style.fontSize =
        Math.max(this._minSize, (1 - d.age) * this._maxSize) + "px";
    }
  },
...
export default fireworksSHOWER;
